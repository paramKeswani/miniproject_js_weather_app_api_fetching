...
const apiKey = 'a0dd0a1fc6166396bfd09c7f89f51525';
const geoApiKey = 'AIzaSyBpbOCN9htyBZByT8sF3ghHj9cJb7O0z9A';
let latitude = 0;
let longitude = 0;

const loadData = (form,loading) => {
    form.style.display = 'none';
    loading.style.visibility = 'visible';
    setInterval(show(form,loading)
...
var hide_btn = document.getElementById('req');
var loading = document.getElementsByClassName('loading-container')[0];
...
document.getElementById('grantAccess').addEventListener('click', function() {
    if (navigator.geolocation) {
        ...
        document.getElementsByClassName('grant-location-container')[0].style.display = 'none';
...
async function yourWeather() {
    ...
    try {
        // var windspeed = document.querySelectorAll('[data-windspeed]')[0];
        // windspeed.textContent =weatherData['wind']['speed'] ;
        
        // var cloudiness = document.querySelectorAll('[data-cloudiness]')[0];
        // cloudiness.textContent = weatherData['clouds']['all'] ;



    let loader = `        <div class="loading-container">
    <img  src="./assets/loading.gif" width="150" height="150">
    <p>Loading</p>
</div>`;
// document.getElementById('weather-container').innerHTML = loader;




        const weatherResponse = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${apiKey}`);
        const geoResponse = await fetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&key=${geoApiKey}`);
        const weatherData = await weatherResponse.json();
        const geoData = await geoResponse.json();
        var loading = document.getElementsByClassName('loading-container')[0];
        var windspeed = document.querySelectorAll('[data-windspeed]')[0];
        windspeed.textContent =weatherData['wind']['speed'] ;
        var humidity = document.querySelectorAll('[data-humidity]')[0];
        humidity.textContent = weatherData['main']['humidity'] ;
        var cloudiness = document.querySelectorAll('[data-cloudiness]')[0];
        cloudiness.textContent = weatherData['clouds']['all'] ;


        var your_city = document.querySelectorAll('[data-cityName]')[0];
var flag_icon = document.querySelectorAll('[data-countryIcon]')[0]; 
        const countryName = geoData['results'][0]['address_components'][8]['long_name'];
        const cityName = geoData['results'][0]['address_components'][2]['long_name'];
        console.log(countryName); 
         your_city.textContent = cityName;
        const container = document.createElement("div");
        // container.innerHTML = `
        //     <div class="col-6" id="country_name">${countryName}</div>
        //     <div class="col-6" id="flag"><img src="" id="flags" alt=""></div>
        // `;
        // document.getElementById('inside').appendChild(container);

        const flagImg = document.getElementById("flags");
        const countryCode = geoData['results'][0]['address_components'][6]['long_name'];
        console.log(countryCode);
        flag_icon.src = countriesFlags[countryName]["mini"];
    } catch (error) {
